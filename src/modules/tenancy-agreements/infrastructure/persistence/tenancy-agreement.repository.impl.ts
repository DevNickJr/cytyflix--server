...
import { TenancyAgreementRepository, PaginatedResult } from "../../contracts/tenancy-agreement.interfaces";
...
export class TenancyAgreementRepositoryImpl implements TenancyAgreementRepository {
  ...
  async create(agreement: TenancyAgreement): Promise<TenancyAgreement> {
    const entity = this.repo.create(TenancyAgreementMapper.toPersistence(agreement));
  ...
  async findById(id: string): Promise<TenancyAgreement | null> {
    const entity = await this.repo.findOne({
      where: { id },
      relations: { landlord: { profile: true }, tenant: { profile: true }, property: true },
    });
    return entity ? TenancyAgreementMapper.toDomain(entity) : null;
  }

  async findByUserId(userId: string, page: number, limit: number): Promise<PaginatedResult<TenancyAgreement>> {
  ...
  async update(agreement: TenancyAgreement): Promise<TenancyAgreement> {
    await this.repo.update(agreement.id, TenancyAgreementMapper.toPersistence(agreement));
