...
export class PropertyRepositoryImpl implements PropertyRepository {
  constructor(private readonly ormRepo: Repository<PropertyOrmEntity>) {}
  ...
  async search(filters: SearchFilters): Promise<PaginatedResult<Property>> {
    ...
    if (filters.city) {
      qb.andWhere('LOWER(property.city) = LOWER(:city)', {
        city: filters.city,
      });
    }

    if (filters.state) {
      qb.andWhere('LOWER(property.state) = LOWER(:state)', {
        state: filters.state,
      });
    }
    ...
    if (filters.listingType) {
      qb.andWhere('property.listingType = :listingType', {
    ...
    if (filters.isAvailable !== undefined) {
      qb.andWhere('property.isAvailable = :isAvailable', {
