...
export class SavedListingController {
  ...
  toggleSave = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this.savedListingService.toggleSave(
        req.user!.id,
        req.body.propertyId
      );
  ...
  getMyListings = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.savedListingService.getSavedListings(
        req.user!.id,
        page,
        limit
      );
  ...
  checkStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this.savedListingService.checkIfSaved(
        req.user!.id,
